                ...
                <ul className="space-y-3">
                    {messages.map((message, index) => (
                        <li key={index} className={`flex ${message.sender ? 'justify-end' : 'justify-start'}`}>
                            ...
                            <div className={`px-4 ml-5 py-2 rounded-xl shadow ${theme === 'dark' ? (message.sender ? 'text-gray-700 bg-gray-100' : 'bg-gray-700 text-gray-100 mr-8') : (message.sender ? 'bg-gray-100' : 'bg-neutral-500 text-gray-50 mr-8')}`}>
